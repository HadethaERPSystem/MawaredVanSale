...
class ItemDataSource(private val api: ApiService, private val term: String, private val priceCode: String):
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, Product>) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = api.products_GetOnPages(term, priceCode, page, POST_PER_PAGE)
                if (response.isSuccessful) {
                    ...
                    if (result.isSuccessful) {
                        ...
                        Log.i("ItemDataSource", "${result.message}")
                    }
                } else {
                    networkState.postValue(NetworkState.ERROR)
                }
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Product>) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = api.products_GetOnPages(term, priceCode, params.key, POST_PER_PAGE)
                if (response.isSuccessful) {
                    val result = response.body()!!
                    if (result.isSuccessful) {
                        if(result.data != null){
