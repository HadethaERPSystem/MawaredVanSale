...
class InventoryDataSource(private val api: ApiService, private val wr_Id: Int, private val dtFrom: String, private val  dtTo: String): PageKeyedDataSource<Int, StockStatement>() {
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, StockStatement>) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = api.inventoryStatement_OnPages(wr_Id, dtFrom, dtTo, page, POST_PER_PAGE)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, StockStatement>) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = api.inventoryStatement_OnPages(wr_Id, dtFrom, dtTo, params.key, POST_PER_PAGE)
                if (response.isSuccessful) {
                    val result = response.body()!!
                    if (result.isSuccessful) {
                        if(result.data != null){
