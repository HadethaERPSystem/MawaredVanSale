package com.mawared.mawaredvansale.utils;
...
public class SunmiPrintHelper {
    ...
    public void printExample(Context context){
        ...
        try {
            ...
            Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_action_add);
            sunmiPrinterService.printBitmap(bitmap, null);
    ...
    public void printReceipt(Context context, List<SunmiTicket> lines){
        if(sunmiPrinterService == null){
            //TODO Service disconnection processing
            return ;
        }
        ...
        try {
            ...
            sunmiPrinterService.setAlignment(0, null);
            try {
                sunmiPrinterService.setPrinterStyle(WoyouConsts.SET_LINE_SPACING, 2);
            } catch (RemoteException e) {
                sunmiPrinterService.sendRAWData(new byte[]{0x1B, 0x33, 0x00}, null);
            }

            for (SunmiTicket l : lines  ) {
                // Set font
                int fnt = l.getFontSize();
                if(fnt > 0){
                    sunmiPrinterService.setFontSize(fnt, null);
                }
                //Text, Image, Barcode, Column
                if(l.isBold()){
                    try {
                        sunmiPrinterService.setPrinterStyle(WoyouConsts.ENABLE_BOLD, WoyouConsts.ENABLE);
                    } catch (RemoteException e) {
                ...
                if(l.getType() == LineType.Text){
                    sunmiPrinterService.printText(l.getText(), null);
                }
                else if(l.getType() == LineType.Image){
                    sunmiPrinterService.printBitmap(l.getBmp(), null);
                    sunmiPrinterService.lineWrap(1, null);
                }
                else if(l.getType() == LineType.Barcode){
                    sunmiPrinterService.setAlignment(1, null);
                    //sunmiPrinterService.printBarCode(l.getText(), 10, 0, null);
                    sunmiPrinterService.printBarCode("{B"+l.getText(),  8, 70, 2, 1, null);
                    sunmiPrinterService.lineWrap(1, null);
                    sunmiPrinterService.lineWrap(1, null);
                    sunmiPrinterService.lineWrap(1, null);
                }
                else if(l.getType() == LineType.Column){
                    sunmiPrinterService.printColumnsString(l.getCols(), l.getWidth(), l.getAlign(), null);
                }
                else if(l.getType() == LineType.Line){
                    if(paper == 1){
                        sunmiPrinterService.printText("--------------------------------\n\n\n", null);
                    }else{
                        sunmiPrinterService.printText("------------------------------------------------\n\n\n",
