...
class SelectDocForStockinViewModel(private val repository: IMDataRepository) : ViewModel() {
    ...
    fun loadData(list: MutableList<InventoryDoc>, term: String, pageCount: Int, loadMore: (List<InventoryDoc>?, Int) -> Unit){
        try {
            Coroutines.ioThenMain({
                val tmp = repository.docforStockin_SearchOnPages(term, _wr_id, stockType, pageCount)
                if(tmp != null){
                    list.addAll(tmp)
                }
