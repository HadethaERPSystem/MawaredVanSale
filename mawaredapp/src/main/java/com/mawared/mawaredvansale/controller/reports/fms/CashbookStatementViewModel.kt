...
class CashbookStatementViewModel(private val repository: ICashbookRepository) : BaseViewModel() {
    ...
    val doSearch: MutableLiveData<Int> = MutableLiveData()

    var dateNavigator: IDateRangePicker? = null

    val cbsItems: LiveData<PagedList<CashbookStatement>> = Transformations.switchMap(dtFrom) {
        repository.fetchLivePagedList(_sm_id, it, dtTo.value)
    }
