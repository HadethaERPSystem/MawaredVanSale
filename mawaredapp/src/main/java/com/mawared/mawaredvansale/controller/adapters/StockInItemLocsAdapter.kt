...
class StockInItemLocsAdapter(@LayoutRes private val layoutResource: Int, private val docLine: InventoryDocLines, private val clickFunc1: (Loc) -> Unit) : BaseAdapter<Loc>(null, layoutResource) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        with(holder.itemView){
            ...
            this.setOnClickListener {
                ...
                //}
                var tQty = 0.0
                if(docLine.itemSelectedLoc != null){
                    tQty = docLine.itemSelectedLoc!!.sumByDouble { it.qty!! }
                }
                item.addQty = docLine.invQty!! - tQty
                if(item.addQty != 0.0){
                    clickFunc1(item)
                }
